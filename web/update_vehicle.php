...
{
    $name_ = $row_vehicle['name'];
    $model_ = $row_vehicle['model'];
}

if (isset($_POST['save_btn'])) {
    $name = $_POST['name'];
    $model = $_POST['model'];
    $sql = "UPDATE  `vehicles` SET `name`='$name',model='$model' WHERE id = $vehicle_id";
    ...
    <div class="form-label-group">
        ...
        <div dir="ltr" class="container-fluid w-75 mt-5" style="border-color: #fff;border-style: solid;border-width: 4px;border-radius: 35px;background-color: white;">
            ...
            <div class="row justify-content-center mt-3" style="margin: auto">
                <div class="col-6">
                    <h4 style="text-align: center;color:#174f96"> Update Vehicle </h4>
                </div>
            </div>

            <form action="update_vehicle.php?id=<?= $vehicle_id;?>" method="post" enctype="multipart/form-data">
                ...
                <div class="row justify-content-center mt-2 mb-2">
                    <div class="col-6 title pt-2">
                        <h3 style="background-color: #004E7C;color: #fff;"> model </h3>
                    </div>
                    <div class="col-6 mt-1">
                        <input name="model" value="<?= $model_; ?>" type="text"    class="w-100 p-2" required>
                    </div>
                </div>
